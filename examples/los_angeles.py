...
# Hollywood Hills, and the Hollywood Sign (~8 km x 8 km at 1 m resolution).
...
def load_terrain():
    ...
    terrain = fetch_dem(
        bounds=BOUNDS,
        output_path=CACHE / "los_angeles_dem.tif",
        source='usgs_1m',
        crs=CRS,
    )
...
if __name__ == "__main__":
    ...
    try:
        bldg_data = fetch_buildings(bounds=BOUNDS, cache_path=CACHE / "los_angeles_buildings.geojson")
        info = ds.rtx.place_buildings(bldg_data, z='elevation', elev_scale=0.5,
                                      mesh_cache=CACHE / "los_angeles_buildings_mesh.npz")
    ...
    try:
        for rt, gid, clr in [('major', 'road_major', (0.10, 0.10, 0.10)),
            ...
            data = fetch_roads(bounds=BOUNDS, road_type=rt,
                               cache_path=CACHE / f"los_angeles_roads_{rt}.geojson")
            info = ds.rtx.place_roads(data, z='elevation', geometry_id=gid, color=clr,
                                      mesh_cache=CACHE / f"los_angeles_roads_{rt}_mesh.npz")
    ...
    try:
        water_data = fetch_water(bounds=BOUNDS, water_type='all',
                                 cache_path=CACHE / "los_angeles_water.geojson")
        results = ds.rtx.place_water(water_data, z='elevation',
                                     mesh_cache_prefix=CACHE / "los_angeles_water")
    ...
    try:
        fire_data = fetch_firms(bounds=BOUNDS, date_span='7d',
                                cache_path=CACHE / "los_angeles_fires.geojson",
                                crs=CRS)
        if fire_data.get('features'):
            with warnings.catch_warnings():
                ...
                fire_info = ds.rtx.place_geojson(
                    fire_data, z='elevation', height=20 * 0.5,
