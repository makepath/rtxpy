...
def load_terrain():
    ...
    terrain = fetch_dem(
        bounds=BOUNDS,
        output_path=CACHE / "capetown_dem.tif",
        source='copernicus',
...
if __name__ == "__main__":
    ...
    try:
        bldg_data = fetch_buildings(bounds=BOUNDS, cache_path=CACHE / "capetown_buildings.geojson")
        info = ds.rtx.place_buildings(bldg_data, z='elevation', elev_scale=0.025,
                                      mesh_cache=CACHE / "capetown_buildings_mesh.npz")
    ...
    try:
        for rt, gid, clr in [('major', 'road_major', (0.10, 0.10, 0.10)),
            ...
            data = fetch_roads(bounds=BOUNDS, road_type=rt,
                               cache_path=CACHE / f"capetown_roads_{rt}.geojson")
            info = ds.rtx.place_roads(data, z='elevation', geometry_id=gid, color=clr,
                                      mesh_cache=CACHE / f"capetown_roads_{rt}_mesh.npz")
    ...
    try:
        water_data = fetch_water(bounds=BOUNDS, water_type='all',
                                 cache_path=CACHE / "capetown_water.geojson")
        results = ds.rtx.place_water(water_data, z='elevation',
                                     mesh_cache_prefix=CACHE / "capetown_water")
    ...
    try:
        fire_data = fetch_firms(bounds=BOUNDS, date_span='7d',
                                cache_path=CACHE / "capetown_fires.geojson",
                                crs='EPSG:32734')
